    ...
    return (
        <div className={cn("flex items-center gap-2 pl-[22px] pr-0 pt-[10px] pb-[5px]", className)}>
            <span className={cn(
                ...
                variant === 'primary' ? "text-[#6C5CE7]" : "text-[#6C5CE7]/70"
            )}>
                {label}
            </span>
            <div className={cn(
                "flex-1 h-[1.5px] transition-colors duration-200",
                variant === 'primary' ? "bg-gradient-to-r from-[#6C5CE7]/40 to-transparent" : "bg-gradient-to-r from-[#6C5CE7]/25 to-transparent"
            )} />
        </div>
    );
}
